...
def create_4photo_magazine_layout(photo1, photo2, photo3, photo4, header_text, bullet_points, phone_number=None, logo_path="assets/logo.png", logo_with_white_bg=False, enable_white_border=True, enable_rounded_corners=True, enable_shadow_effect=True, shadow_strength=50, background=None):
    """
    Simple Style Layout - 4 nuotraukos horizontaliai + antraštė viršuje per centrą + bullet'ai po kiekviena nuotrauka
    
    Args:
        photo1, photo2, photo3, photo4: PIL Images
        header_text: Antraštės tekstas (56px, centruotas)
        bullet_points: List of strings arba string su \n (4 punktai - po vieną po kiekviena nuotrauka)
        phone_number: Telefono numeris (optional)
        logo_path: Kelias iki logo (optional)
        logo_with_white_bg: True = baltas fonas, False = permatomas
    ...
    if background is not None:
        ...
        canvas = Image.new('RGBA', (1327, 768), color=(250, 246, 239, 255))
    
    draw = ImageDraw.Draw(canvas)
    
    # Automatinė teksto spalva pagal fono šviesumą
    def get_text_color_for_background(canvas_image):
        # Sample iš viso canvas centro
        text_area = canvas_image.crop((300, 100, 1000, 400))
        if text_area.mode != 'RGB':
            text_area = text_area.convert('RGB')
        pixels = list(text_area.getdata())
        avg_brightness = sum(r * 0.299 + g * 0.587 + b * 0.114 for r, g, b in pixels) / len(pixels)
    ...
    if logo_path and os.path.exists(logo_path):
        try:
            ...
            if logo_with_white_bg:
                logo_bg = Image.new('RGBA', (120, 120), (255, 255, 255, 255))
                logo_bg.paste(logo, (10, 10), logo)
                canvas.paste(logo_bg, (30, 22), logo_bg)
            else:
                logo_data = logo.getdata()
    ...
    # Centruojame antraštę
    bbox = draw.textbbox((0, 0), header_text, font=font_header)
    header_width = bbox[2] - bbox[0]
    header_x = (1327 - header_width) // 2
    draw.text((header_x, 60), header_text, fill=text_color, font=font_header)
    
    # === 4 NUOTRAUKOS HORIZONTALIAI ===
    # Skaičiavimai: 4 nuotraukos, padding 40px šonuose, tarpai 20px
    # Plotis: (1327 - 80 - 60) / 4 = ~296px
    # Aukštis: ~395px (3:4 ratio)
    photo_width = 285
    photo_height = 380
    start_x = 40
    gap = 20
    ...
    for i, photo in enumerate(photos):
        ...
        photo_with_effects = add_photo_effects(
            photo_resized,
            enable_border=enable_white_border,
            border_width=12,
            enable_rounded=enable_rounded_corners,
        ...
        canvas.paste(photo_with_effects, (x_pos, photo_y), photo_with_effects if photo_with_effects.mode == 'RGBA' else None)
    ...
    for font_path in bullet_fonts:
        try:
            font_bullet = ImageFont.truetype(font_path, 20)
            break
        except:
            continue
    if not font_bullet:
    ...
    # Užtikriname 4 punktus
    while len(bullets) < 4:
        bullets.append("Tekstas")
    bullets = bullets[:4]
    
    # Bullet'ai po kiekviena nuotrauka
    bullet_y = photo_y + photo_height + 25
    
    for i, bullet_text in enumerate(bullets):
        x_pos = start_x + i * (photo_width + gap)
        photo_center_x = x_pos + photo_width // 2
        
        # Apskritimas
        circle_radius = 8
        circle_x = photo_center_x - 50
        circle_y = bullet_y + 8
        ...
        # Tekstas šalia apskritimo
        text_x = circle_x + circle_radius + 10
        draw.text((text_x, bullet_y), bullet_text.upper(), fill=text_color, font=font_bullet)
    ...
    if phone_number:
        ...
        bbox = draw.textbbox((0, 0), phone_number, font=font_phone)
        phone_width = bbox[2] - bbox[0]
        phone_x = (1327 - phone_width) // 2
        phone_y = 720
        draw.text((phone_x, phone_y), phone_number, fill=text_color, font=font_phone)
